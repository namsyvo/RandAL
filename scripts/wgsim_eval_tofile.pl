...
sub alneval {
	my %opts = (g=>20);
	getopts('pagf:', \%opts); # f: to get path to result files - added by Nam S. Vo
	die(qq/
...
\n/) if (@ARGV == 0 && -t STDIN);
	...
	# -------------------------------------------------
	# for writing results to files - added by Nam S. Vo
	my $inputFile = $path.$ARGV[0];
	#my $fpFile = $path."FP/".$ARGV[0];
	my $evalFile = $path."Eval/accuracy.txt";
	open (INPUTFILE, "$inputFile");
	#open (FPFILE, ">>$fpFile");
	open (EVALFILE, ">>$evalFile");
	# -------------------------------------------------

	while (<INPUTFILE>) { # indicate the explicit inputfile - added by Nam S. Vo
		next if (/^\@/);
		my @t = split("\t");
		next if (@t < 11);
		my $line = $_;
		...
		#print STDERR $line if (($flag&1) && !$is_correct && $q > 0); #commented by Nam S. Vo
		#print FPFILE $line if (($flag&1) && !$is_correct && $q > 0); # added by Nam S. Vo
	}
	# print
	my ($cc0, $cc1) = (0, 0);
	if (!defined($opts{a})) {
		for (my $i = $max_q; $i >= 0; --$i) {
			...
			$cc0 += $c0[$i]; $cc1 += $c1[$i];
			#printf("%.2dx %12d / %-12d  %12d  %.3e\n", $i, $c1[$i], $c0[$i], $cc0, $cc1/$cc0) if ($cc0); #commented by Nam S. Vo
		}
		printf EVALFILE ("%d\t%d\n", $c1[25], $c0[25]) if ($cc0);  # added by Nam S. Vo
	} else {
		for (reverse(sort {$a<=>$b} (keys %fnfp))) {
			next if ($_ == 0);
			$cc0 += $fnfp{$_}[0];
			$cc1 += $fnfp{$_}[1];
			print EVALFILE join("\t", $_, $cc0, $cc1), "\n";
		}
	}
